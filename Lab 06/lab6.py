...
def has_sublist(s, sublist):
    ...
    False
    """
    # loop thru and find if sublist appears exactly in linked list s, use rest to find, ignore head?
    ...
    if sublist == empty:
        ...
        return has_prefix(rest(s), rest(sublist))
    else:
...
def has_kitty_gene(dna):
    ...
    if has_sublist(dna, link('C', link('A', link('T', link('C', link('A', link('T'))))))) == True:
        return True
    else:
        return False
