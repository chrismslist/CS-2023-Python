...
#RQ2
    def go_to(self, direction):
        ...
        #print(self.place.exits[direction][0].name)
        
        for place in self.place.exits:
            #print(direction)
            #print(place)
            if direction == place:
                self.place = self.place.exits[place][0]
                print('You are at {}'.format(place))
                return None
...
#RQ3
    def talk_to(self, person):
        ...
        for character in self.place.characters:
            temp = self.place.characters[character]
            if person == character:
                print('{} says: {}'.format(temp.message, temp.name))
                return None
